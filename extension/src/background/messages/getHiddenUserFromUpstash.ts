...
const handler: PlasmoMessaging.MessageHandler = async (req, res) => {
    try {
        const { userDecodedId } = req.body;
        const userData = await upstashRequest('hgetall', `hiddenUsers:${userDecodedId}`);
        console.log(userData);  // { profileUrl: "URL_HERE", profilePicUrl: "URL_HERE" }        
